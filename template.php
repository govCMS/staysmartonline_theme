...
require_once drupal_get_path('theme', 'stay_smart_online') . '/template.helpers.inc';

/**
 * Implements hook_preprocess_page().
 */
function stay_smart_online_preprocess_html(&$variables, $hook) {
 ...
 */
function stay_smart_online_js_alter(&$javascript) {
 ...
 * Implements hook_preprocess_page().
 */
function stay_smart_online_preprocess_page(&$variables, $hook) {
  ...
  if (theme_get_setting('feedback_enabled') && !empty(theme_get_setting('feedback_wform_nid'))) {
    ...
    $variables['site_pages_feedback_form'] = _stay_smart_online_webform_render($wf_nid);
  }

  // Create template variables for the header menu block.
  $variables['header_search'] = _stay_smart_online_block_render('search', 'form');
  $variables['header_menu'] = _stay_smart_online_block_render('system', 'main-menu');
  // Create template variables for the footer menu blocks.
  $variables['footer_menu'] = _stay_smart_online_block_render('menu', 'menu-footer-menu');
  $variables['footer_auxilary_menu'] = _stay_smart_online_block_render('menu', 'menu-footer-sub-menu');
 ...
 */
function stay_smart_online_get_standard_page_menu_children($item) {
 ...
 */
function stay_smart_online_preprocess_entity(&$variables, $hook) {
  if ($variables['entity_type'] === 'bean' && $variables['bean']->type === 'standard_page_children' && $variables['view_mode'] === 'coloured_links_grid') {
    ...
    $nids = stay_smart_online_get_standard_page_menu_children($item);
 ...
 */
function _stay_smart_online_related_content_category_term(&$related_content_nids, $limit, $node, $field_name) {
 ...
 *   Render Array.
 */
function _stay_smart_online_related_content($node) {
  $limit = 4;
  ...
  _stay_smart_online_related_content_category_term($related_content_nids, $limit, $node, 'field_business_area');

  // Next fill related content with content of same type in this stream.
  _stay_smart_online_related_content_category_term($related_content_nids, $limit, $node, 'field_stream');

  // Next fill related content with content of same type in this audience.
  _stay_smart_online_related_content_category_term($related_content_nids, $limit, $node, 'field_audience');
 ...
 */
function stay_smart_online_preprocess_node(&$variables, $hook) {
  $node = $variables['node'];
  ...
  if (in_array($variables['type'], array(
    ...
    if (isset($hide_related_content) && !$hide_related_content) {
      $variables['content']['related_content'] = _stay_smart_online_related_content($variables['node']);
    }
  }
 ...
 */
function stay_smart_online_preprocess_views_view_field(&$variables) {
 ...
 */
function stay_smart_online_form_alter(&$form, &$form_state, $form_id) {
 ...
 */
function stay_smart_online_read_more_link($href, $text, $external = FALSE) {
  $template_file = drupal_get_path('theme', 'stay_smart_online') . '/templates/read-more-link.tpl.php';
 ...
 */
function stay_smart_online_preprocess_block(&$variables) {
 ...
 */
function _stay_smart_online_block_render($module, $delta) {
  $output = '';
 ...
 */
function stay_smart_online_menu_tree__main_menu($variables) {
 ...
 */
function stay_smart_online_menu_link__main_menu(array $variables) {
 ...
 * Implements theme_menu_tree__MENU_NAME().
 */
function stay_smart_online_menu_tree__menu_footer_menu($variables) {
 ...
 * Implements theme_menu_link__MENU_NAME().
 */
function stay_smart_online_menu_link__menu_footer_menu(array $variables) {
  $element = $variables['element'];
  $sub_menu = '';
 ...
 * Implements theme_menu_tree__MENU_NAME().
 */
function stay_smart_online_menu_tree__menu_footer_sub_menu($variables) {
 ...
 * Implements theme_menu_link__MENU_NAME().
 */
function stay_smart_online_menu_link__menu_footer_sub_menu(array $variables) {
  $element = $variables['element'];
  $sub_menu = '';
 ...
 */
function stay_smart_online_file_icon($variables) {
  $file = $variables['file'];
  $icon_directory = drupal_get_path('theme', 'stay_smart_online') . '/images/document';
 ...
 */
function stay_smart_online_breadcrumb($variables) {
 ...
 */
function stay_smart_online_trim($markup, $trim_length) {
 ...
 */
function stay_smart_online_preprocess_field(&$variables, $hook) {
  $element =& $variables['element'];
  ...
  if (($element['#field_type'] === 'text_with_summary' || $element['#field_type'] === 'text_long') && ($element['#formatter'] === 'text_summary_or_trimmed' || $element['#formatter'] === 'text_trimmed')) {
    ...
    foreach ($element['#items'] as $delta => $item) {
      ...
      $variables['items'][$delta]['#markup'] = stay_smart_online_trim($markup, $trim_length);
    }
  }
 ...
 */
function stay_smart_online_ds_pre_render_alter(&$layout_render_array, $context, &$variables) {
 ...
 */
function stay_smart_online_preprocess_poll_results(&$variables) {
 ...
 */
function stay_smart_online_preprocess_views_view(&$variables) {
 ...
 */
function stay_smart_online_theme($existing, $type, $theme, $path) {
  return array(
    'share_row' => array(
 ...
 * Implements hook_theme().
 */
function stay_smart_online_item_list($variables) {
 ...
 */
function stay_smart_online_pager($variables) {
  $tags = $variables['tags'];
  $element = $variables['element'];
 ...
 * @param $langcode
 */
function stay_smart_online_node_view_alter(&$build) {
 ...
 * @ingroup themeable
 */
function stay_smart_online_facetapi_link_active($variables) {
  ...
  return $link_text . '<a href="' . check_plain(url($variables['path'], $variables['options'])) . '"'
        ...
        //  . ($variables['options']['html'] ? $variables['text'] : check_plain($variables['text']))
         . '    <img src="' . drupal_get_path('theme', 'stay_smart_online') . '/images/close.svg"'
